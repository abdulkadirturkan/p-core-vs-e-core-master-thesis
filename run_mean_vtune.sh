#!/bin/bash
# ============================================================
# VTune Profiling + 100-Run Benchmark — Mean Filter
# ============================================================
...
if [[ $paranoid -gt 0 ]]; then
    ...
    echo "Düzelt: sudo sysctl -w kernel.perf_event_paranoid=0"
    exit 1
fi
...
if [[ $ptrace_scope -gt 0 ]]; then
    ...
    echo "Düzelt: sudo sysctl -w kernel.yama.ptrace_scope=0"
    exit 1
fi
...
BASE_DIR="./vtune_results/mean_${TIMESTAMP}"

mkdir -p "$BASE_DIR"

ln -sfn "mean_${TIMESTAMP}" ./vtune_results/latest
...
run_benchmark() {
    ...
    if (( successful_runs > 0 )); then
        ...
        {
            echo "==========================================="
            echo "Core: $core_name | Filter: mean | Radius: $size | Threads: $thread"
            echo "Runs: $runs"
...
echo "============================================================"
echo "VTune + 100-Run Benchmark — Mean Filter"
echo "Tarih    : $(date)"
